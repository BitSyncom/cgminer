...
int opt_avalon9_freq[AVA9_DEFAULT_PLL_CNT] = {AVA9_DEFAULT_FREQUENCY_0,
					      AVA9_DEFAULT_FREQUENCY_1,
					      AVA9_DEFAULT_FREQUENCY_2,
					      AVA9_DEFAULT_FREQUENCY_3};
...
static int decode_pkg(struct cgpu_info *avalon9, struct avalon9_ret *ar, int modular_id)
{
	...
	case AVA9_P_STATUS_PVT:
		applog(LOG_DEBUG, "%s-%d-%d: AVA9_P_STATUS_PVT", avalon9->drv->name, avalon9->device_id, modular_id);
		{
			...
			for (i = 0; i < AVA9_DEFAULT_CORE_VOLT_CNT; i++) {
...
static void detect_modules(struct cgpu_info *avalon9)
{
	...
	for (i = 1; i < AVA9_DEFAULT_MODULARS + 1; i++) {
		...
		memset(info->set_frequency[i], 0, sizeof(unsigned int) * info->miner_count[i] * AVA9_DEFAULT_PLL_CNT);
...
static float avalon9_hash_cal(struct cgpu_info *avalon9, int modular_id)
{
	...
	mhsmm = 0;
	for (i = 0; i < info->miner_count[modular_id]; i++) {
		for (j = 0; j < AVA9_DEFAULT_PLL_CNT; j++)
		...
		if (opt_debug) {
			...
			for (j = 0; j < info->miner_count[i]; j++) {
				for (k = 0; k < info->asic_count[i]; k++) {
					...
					strcat(statbuf, buf);
					for (m = 0; m < AVA9_DEFAULT_CORE_VOLT_CNT; m++) {
...
char *set_avalon9_factory_info(struct cgpu_info *avalon9, char *arg)
{
	...
	sscanf(arg, "%d", &val);
	if (!val)
		val = AVA9_DEFAULT_FACTORY_INFO_0;

	if (val < AVA9_DEFAULT_FACTORY_INFO_0_MIN || val > AVA9_DEFAULT_FACTORY_INFO_0_MAX)
