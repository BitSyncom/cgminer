...
int opt_avalon8_core_otp = AVA8_INVALID_CORE_OTP;

static uint8_t opt_avalon8_cycle_hit_flag = 0;
...
char *set_avalon8_core_otp(char *arg)
{
       int val, ret;
       char buf[70], *buf_ptr;
       ...
       ret = sscanf(arg, "%d", &val);
       if (ret < 1)
               return "No value passed to avalon8-core-otp";

       if (val < 0 || val > (AVA8_DEFAULT_ASIC_MAX-1)){
	       sprintf(buf, "Invalid value %d passed to avalon8-core-otp! Valid range: 0-%d", val,(AVA8_DEFAULT_ASIC_MAX-1));
               return buf_ptr;
       }

       opt_avalon8_core_otp = val;

       opt_avalon8_cycle_hit_flag = 0; /*reset cycle_hit_flag each time for configuration*/
...
static int decode_pkg(struct cgpu_info *avalon8, struct avalon8_ret *ar, int modular_id)
{
	...
	case AVA8_P_STATUS_OTP:
		/*use this flag to avoid too much resource waste*/
		if (1 == opt_avalon8_cycle_hit_flag)
		      break;
		
		applog(LOG_DEBUG, "%s-%d-%d: AVA8_P_STATUS_OTP", avalon8->drv->name, avalon8->device_id, modular_id);
		
		/*data[17], ASIC reading cycle limit hitting flag, sent from MM to cgminer*/
		if ( 1 == ar->data[17]){
		...
		/* 
		 * pkg->data[15]: the reading step flag on MM side, 0:byte[3:0], 1:byte[7:4], 2: byte[11:8]
		 * pkg->data[16]: the asic selected to read
		 */
		if(0 == ar->data[15])
			memcpy(info->otp_info[miner_id], ar->data, 4);
		else if (1 == ar->data[15])
			memcpy(info->otp_info[miner_id] + 4, ar->data + 4, 4);
		else if (2 == ar->data[15])
...
static void detect_modules(struct cgpu_info *avalon8)
{
	...
	for (i = 1; i < AVA8_DEFAULT_MODULARS + 1; i++) {
		...
		for (j = 0; j < info->miner_count[i]; j++) {
			...
			/*core setting for otp reading*/	
			if (opt_avalon8_core_otp == AVA8_INVALID_CORE_OTP)
				info->set_core_otp[i][j] = 0; /*default value: 0*/
			else
				info->set_core_otp[i][j] = opt_avalon8_core_otp;
		}
...
static void avalon8_set_core_otp(struct cgpu_info *avalon8, int addr, unsigned int core[])
{
	struct avalon8_info *info = avalon8->device_data;
	struct avalon8_pkg send_pkg;
	...
	for (i = 0; i < info->miner_count[addr]; i++) {
		...
		if(core[i] < 0)
		      core[i] = 0;
		else if (core[i] > (AVA8_DEFAULT_ASIC_MAX -1))
			core[i] = AVA8_DEFAULT_ASIC_MAX - 1;
		tmp = be32toh(core[i]);
		memcpy(send_pkg.data + i * 4, &tmp, 4);
	}
	applog(LOG_DEBUG, "%s-%d-%d: avalon8 set core for otp reading %d, (%d-%d)",
			avalon8->drv->name, avalon8->device_id, addr,
			i, core[0], core[info->miner_count[addr] - 1]);

	/* Package the data */
	avalon8_init_pkg(&send_pkg, AVA8_P_SET_CORE_OTP, 1, 1);
	if (addr == AVA8_MODULE_BROADCAST)
		avalon8_send_bc_pkgs(avalon8, &send_pkg);
	else
		avalon8_iic_xfer_pkg(avalon8, addr, &send_pkg, NULL);
...
static int64_t avalon8_scanhash(struct thr_info *thr)
{
	...
	for (i = 1; i < AVA8_DEFAULT_MODULARS; i++) {
		...
		if (update_settings) {
			...
			avalon8_set_core_otp(avalon8, i, info->set_core_otp[i]);
			for (j = 0; j < info->miner_count[i]; j++)
